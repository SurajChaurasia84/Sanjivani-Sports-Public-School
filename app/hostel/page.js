      ...
      <Section title="Overview" subtitle="Everything you need to know about staying at SSPS.">
        <div className="flex flex-col md:flex-row gap-16 items-center">
          <div className="flex-1">
            <div className="relative p-1 bg-gradient-to-br from-emerald-500 to-blue-600 rounded-[2.5rem]">
              <img 
              ...
              />
              <div className="absolute -bottom-6 -right-6 bg-white p-6 rounded-3xl shadow-xl border border-emerald-50 hidden lg:block">
                <p className="text-3xl font-bold text-emerald-900 leading-tight">Comfortable <br/><span className="text-emerald-500">Living</span></p>
              </div>
            </div>
          </div>
          <div className="flex-1 space-y-8">
            ...
            <div className="space-y-6">
              <div className="flex gap-4 p-6 bg-slate-50 rounded-2xl border border-slate-100 transform transition-transform hover:scale-[1.02]">
                <div className="bg-emerald-600 text-white w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0">
                  <Star size={24} />
                </div>
                <div>
          ...
          {data.facilities.map((facility, idx) => {
            ...
            return (
              <Card 
                ...
                icon={Icon}
                className="bg-white border-0 hover:border-b-4 hover:border-emerald-500"
              />
            );
          })}
        </div>
            ...
            <div className="relative z-10 space-y-6">
              {[
                ...
                <div key={idx} className="flex items-center gap-8 group">
                  <div className="min-w-[100px] text-emerald-400 font-bold group-hover:text-white transition-colors">{item.time}</div>
                  <div className="h-0.5 flex-grow bg-emerald-800 group-hover:bg-emerald-600 transition-colors"></div>
                  <div className="min-w-[250px] font-medium text-slate-200 group-hover:text-white transition-colors">{item.activity}</div>
                </div>
              ))}
            </div>
            {/* Decoration */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-emerald-600 rounded-full blur-[120px] opacity-20 -z-0"></div>
          </div>
        </div>
      </Section>
    </div>
