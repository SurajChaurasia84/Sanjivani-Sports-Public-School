    ...
    <div className="bg-slate-50">
      ...
      <div className="bg-emerald-900 py-24 text-white relative overflow-hidden">
        <div className="container mx-auto px-4 relative z-10 text-center">
          ...
          <p className="text-xl text-emerald-100 max-w-2xl mx-auto opacity-80 leading-relaxed font-medium">
      ...
      <Section>
        <div className="max-w-4xl mx-auto space-y-6">
          {notices.map((notice) => (
            <div 
              key={notice.id} 
              className={`bg-white rounded-3xl p-8 shadow-sm border border-slate-100 transition-all duration-300 hover:shadow-xl hover:border-emerald-200 group relative overflow-hidden`}
            >
              {notice.isUrgent && (
              ...
              <div className="flex flex-col md:flex-row gap-8 items-start">
                <div className="flex-shrink-0 flex flex-col items-center bg-slate-50 p-6 rounded-2xl border border-slate-100 group-hover:bg-emerald-50 group-hover:border-emerald-100 transition-colors">
                  <span className="text-3xl font-bold text-slate-800 group-hover:text-emerald-700 transition-colors">
                ...
                <div className="flex-grow">
                  <div className="flex flex-wrap items-center gap-3 mb-3">
                    <h2 className="text-2xl font-bold text-slate-900 group-hover:text-emerald-900 transition-colors">{notice.title}</h2>
                    {notice.isUrgent && (
          ...
          <div className="text-center pt-12 border-t border-slate-200">
            ...
            <Link href="/" className="text-emerald-600 font-bold mt-4 inline-block hover:underline">Return to Homepage</Link>
          </div>
        </div>
      </Section>

      {/* Archive Section Callout */}
      <Section className="bg-emerald-900 text-white p-2">
         <div className="container mx-auto px-4 text-center py-12">
            <h3 className="text-3xl font-bold font-display mb-4">Looking for past announcements?</h3>
            <p className="text-emerald-100 opacity-80 mb-8 max-w-xl mx-auto">
              Our digital archive contains notices dating back to 2015. Contact the school office for access to older records.
            </p>
            <button className="bg-emerald-600 text-white px-8 py-3 rounded-full font-bold hover:bg-emerald-500 transition-all border border-emerald-400">
               Access Archive
            </button>
         </div>
      </Section>
